...
class FieldType(Enum):
    """Types of board fields.
    
    Currently only the following field types are supported: SHIP, SEA,
    and UNKNOWN.
    """
...
Ship = NamedTuple(
    "Ship", [("row", int), ("col", int), ("length", int), ("direction", Direction)]
)
"""Battleship uniquely defined by its coordinates, length and
direction.
"""


FieldsShipsMappings = Dict[Position, List[Ship]]
"""Dictionary of fields (keys) and corresponding list of ships that
occupy that field (values).
"""


class Fleet(
    NamedTuple(
        "Fleet", [("ship_lengths", List[int]), ("subfleet_sizes", Dict[int, int])]
    )
):
    """Represents a fleet of ships of different lenghts.
    
    Each length in ship_length (key) is mapped to a number of ships of
    that length (value).
    """
...
class Board:
    ...
    A board consists of a playfield whose every row and column is
    assigned two numbers:
        1) number of undetected ship pieces in a particular row or
            column,
        2) total number of ship pieces in a particular row or column.
    
    For practical reasons the playfield contains one additional row and
    one additional column at the beginning and the end of the playfield.
    Therefore, the playfield area which may contain ship pieces is
    within the range [1 : rows-1][1 : columns-1].
    ...
    Class instance attributes:
        ...
        total_pcs_in_rows (List[int]): List of total required number of
            ship pieces in each playfield row.
        total_pcs_in_cols (List[int]): List of total required number of
            ship pieces in each playfield column.
    """
    ...
    def get_board_from_game_data(
        ...
        """Create a new board initialized with actual game data.
        
        The new board contains sea fields only.
        
        Args:
            playfield (PlayField): Board playfield area which may
                contain ship pieces.
            solution_pcs_in_rows (List[int]): List of ship pieces in
                each playfield row.
            solution_pcs_in_cols (List[int]): List of ship pieces in
    ...
    def repr(self, total_pcs_only: Optional[bool] = False) -> str:
        """Create a convenient string representation of this object.
        
        Args:
            total_pcs_only (bool): Flag indicating whether only total
                required number of ship pieces will be included in the
                representation or both actual and required number of
                ship pieces. Defaults to False, in which case both
    ...
    def __repr__(self) -> str:
        """Override the built-in class method."""
        return self.repr()
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, self.__class__):
            ...
            return (
                other.playfield == self.playfield
                and other.rem_pcs_in_rows == self.rem_pcs_in_rows
                and other.rem_pcs_in_cols == self.rem_pcs_in_cols
                and other.total_pcs_in_rows == self.total_pcs_in_rows
                and other.total_pcs_in_cols == self.total_pcs_in_cols
            )
        return False
...
def get_redefined_fieldtypes(config: ConfigParser) -> Enum:
    """Get a modified FieldType enum definition based on the game
...
def get_ship_pcs_positions(board: Board) -> List[Position]:
    """Determine which board fields contain a ship piece.
...
def mark_uncovered_sea_positions(
    board: Board, ship_pcs_positions: List[Position]
) -> None:
    """Mark sea in those board fields where there can be nothing else
    but sea.
    ...
    Args:
        ...
        ship_pcs_positions (List[Position]): Coordinates of board
            fields containing any ship piece.
    """
...
def can_fit_ship_on_board(board: Board, ship: Ship) -> bool:
    """Check whether a single ship can be placed onto the board without
    ...
    if ship.direction == Direction.RIGHT:
        ship_does_not_fit_onto_the_board = ship.col + ship.length > board.size - 1
        insufficient_rem_pcs_left = ship.length > board.rem_pcs_in_rows[ship.row]
    else:
        ship_does_not_fit_onto_the_board = ship.row + ship.length > board.size - 1
        insufficient_rem_pcs_left = ship.length > board.rem_pcs_in_cols[ship.col]

    if ship_does_not_fit_onto_the_board or insufficient_rem_pcs_left:
        return False

    if ship.direction == Direction.RIGHT:
        invalid_ship_placement = any(
            board.playfield[ship.row][col] != FieldType.UNKNOWN
            for col in range(ship.col, ship.col + ship.length)
        )
        ship_touches_another_ship = (
            board.playfield[ship.row][ship.col - 1] == FieldType.SHIP
            or board.playfield[ship.row][ship.col + ship.length] == FieldType.SHIP
            or any(
        ...
        ship_touches_another_ship = (
            board.playfield[ship.row - 1][ship.col] == FieldType.SHIP
            or board.playfield[ship.row + ship.length][ship.col] == FieldType.SHIP
            or any(
    ...
    if invalid_ship_placement or ship_touches_another_ship:
        return False
    return True


def reduce_rem_pcs_and_mark_sea(board: Board, ship: Ship) -> None:
    """Check for each ship piece whether corresponding row and column
    are filled with the maximum allowed number of ship pieces. If yes,
    mark sea in remaining row/column fields with UNKNOWN.
    
    Args:
        board (Board): The board to check and possibly update.
        ship (Ship): The ship whose placement (rows and columns) to
            check.
...
def mark_ships(
    ...
    Modifies the board and fleet in-place. After placing each ship,
    the remaining fleet is updated accordingly.
...
def get_field_ships(board: Board, row: int, col: int, ship_length: int) -> List[Ship]:
    """Determine ships of length ship_length that can each, one at a
    time, possibly be placed onto the board so that each ship occupies
    the field at coordinates (row, col).
    ...
    Returns:
        List[Ship]: List of ships of length ship_length that can
            possibly be placed onto the board so that each ship
            occupies the board field at coordinates (row, col).
...
def get_fields_ships_mappings(
    board: Board, fleet: Fleet, ship_pcs_positions: List[Position]
) -> FieldsShipsMappings:
    """For each field in ship_pcs_positions determines a list of ships
    that can each potentially be placed onto the board so that a piece
    of that ship occupies that same field.
    ...
    Returns:
        FieldsShipsMappings: Dictionary of ship fields (keys) and
        corresponding list of ships that can possibly occupy that field
        (values).
    ...
    for row, col in ship_pcs_positions:
        for ship_length in fleet.ship_lengths:
            if fleet.subfleet_sizes[ship_length] > 0:
                ships_for_fields[(row, col)] = ships_for_fields.get(
                    (row, col), []
...
def remove_ship_from_mappings(
    ...
    """Remove from fields_ships_mappings all occurences of ship.
    
    If after removing the ship there are no other possible ships for
    some coordinates, remove the coordinates and associated empty list
    from the ships list corresponding to those coordinates. Modifies
    fields_ships_mappings in-place.
    
    Args:
        ship_pcs_positions (List[Position]): Coordinates of board fields
            containing ship pieces.
        fields_ships_mappings (FieldsShipsMappings): Dictionary of ship
            fields (keys) and corresponding list of ships that can
            possibly occupy that field (values).
    ...
    for row, col in ship_pcs_positions:
        ship_occurrence = (
            ...
            and row == ship.row
            and ship.col <= col < ship.col + ship.length
        ) or (
            ship.direction == Direction.DOWN
            and col == ship.col
            and ship.row <= row < ship.row + ship.length
        )
        if ship_occurrence:
            fields_ships_mappings[(row, col)].remove(ship)
            if not fields_ships_mappings[(row, col)]:
...
def mark_uncovered_ships(
    ...
    """Where a particular field can be occupied with only one ship,
    mark that ship onto the board and remove the ship from the fleet.
    
    Modifies the board and fleet objects in-place.
    ...
    for row, col in ship_pcs_positions:
        if len(fields_ships_mappings[(row, col)]) == 1:
            ...
            remove_ship_from_mappings(ship_pcs_positions, fields_ships_mappings, ship)


def get_ships_of_length(board: Board, ship_length: int) -> List[Ship]:
    """Search for ships of length ship_length that can be placed
    ...
    Returns:
        List[Ship]: List of ships of length ship_length that can be
            placed onto the board.
    ...
    for row in range(1, board.size - 1):
        for col in range(1, board.size - 1):
            if board.playfield[row][col] == FieldType.UNKNOWN:
                if (
                    ship_length <= board.rem_pcs_in_rows[row]
                    and col + ship_length < board.size
                ):
                    ship = Ship(row, col, ship_length, Direction.RIGHT)
                    if can_fit_ship_on_board(board, ship):
                        ships.append(ship)
                if (
                    1 < ship_length <= board.rem_pcs_in_cols[col]
                    and row + ship_length < board.size
                ):
...
def find_solutions_for_subfleet(
    board: Board, fleet: Fleet, subfleet_index: int, solutions: List[Board]
) -> None:
    """Search for solutions for given board and given fleet and given
    subfleet index.
    
    If last remaining subfleet has been marked, add the board to the
    list of solutions.
    ...
    if subfleet_size == 0:
        if subfleet_index + 1 < len(fleet.ship_lengths):
            find_solutions_for_subfleet(board, fleet, subfleet_index + 1, solutions)
        else:
            solutions.append(board)
    else:
        ships = get_ships_of_length(board, fleet.ship_lengths[subfleet_index])
...
def get_solutions_for_mappings(
    board: Board, fleet: Fleet, fields_ships_mappings: FieldsShipsMappings
) -> List[Board]:
    """For each field in fields_ships_mappings a ship from the
    corresponding ship list is selected in order to mark onto the board
    and occupy that field. When all fields are successfully covered,
    removes the used ships from the fleet and searches for solutions for
    the partially covered board and the remaining fleet.
    
    Modifies the board and fleet object in-place.
    ...
    def ship_combination_exceeds_fleet(
        ...
        for ship in ship_combination:
            ...
            if combination_fleet_sizes[ship.length] > fleet.subfleet_sizes[ship.length]:
                return True
        return False
    ...
    if not fields_ships_mappings:
        ...
        for ship_combination in product(*fields_ships_mappings.values()):
            if not ship_combination_exceeds_fleet(fleet, set(ship_combination)):
                new_board, new_fleet = board.get_copy(), fleet.get_copy()
                if mark_ships(new_board, new_fleet, ship_combination):
                    find_solutions_for_subfleet(new_board, new_fleet, 0, solutions)
    return solutions
